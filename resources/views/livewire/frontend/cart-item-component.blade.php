...
{{--<tr>--}}
    <th class="pl-0 border-0" scope="row">
        <div class="media align-items-center"><a
                ...
                href="{{route('frontend.product' , $cartItem->model->slug)}}"><img
                    src="{{asset('assets/products/' . $cartItem->model->firstMedia->file_name )}}"
                    alt="{{ $cartItem->model->name }}" width="70"/></a>
            <div class="media-body ml-3"><strong class="h6"><a
                        class="reset-anchor animsition-link"
                        href="{{route('frontend.product' , $cartItem->model->slug)}}">{{ $cartItem->model->name }}</a></strong>
            </div>
        </div>
    </th>
    ...
    <td class="align-middle border-0">
        <a wire:click.prevent="removeFromCart('{{ $cartItem->rowId }}')" x-on:click="show = false"  style="cursor: pointer;"  class="reset-anchor">
