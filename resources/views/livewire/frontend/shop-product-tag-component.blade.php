                ...
                <div class="col-lg-3 order-2 order-lg-1">
                    ...
                    @foreach($shop_categories_menu as $shop_category)
                        ...
                        <ul class="list-unstyled small text-muted pl-lg-4 font-weight-normal">
                            @forelse($shop_category->appearedChildren as $sub_shop_category)
                                <li class="mb-2">
                                    <a class="reset-anchor" href="{{route('frontend.shop' , $sub_shop_category->slug)}}">{{$sub_shop_category->name}}</a>
                                </li>
                            @empty
                    ...
                    <ul class="list-unstyled small text-muted pl-lg-4 font-weight-normal">
                        @forelse($shop_tags_menu as $shop_tag)
                            <li class="mb-2">
                                <a class="reset-anchor" href="{{route('frontend.shop_tag' , $shop_tag->slug)}}">{{$shop_tag->name}}</a>
                            </li>
                        @empty
                ...
                <div class="col-lg-9 order-1 order-lg-2 mb-5 mb-lg-0">
                    <div class="row mb-3 align-items-center">
                        <div class="col-lg-6 mb-2 mb-lg-0">
                            <p class="text-small text-muted mb-0">Showing {{ $products->firstItem() }}–{{ $products->firstItem() }} of {{ $products->total() }} results</p>
                        </div>
                        <div class="col-lg-6">
                            ...
                            <div class="col-4" id="products-container-area">
                                <div class="product text-center">
                                    <div class="mb-3 position-relative">
                                        <div class="badge text-white badge-"></div><a class="d-block" href="{{'frontend.product' , $product->slug}}"><img class="img-fluid w-100" src="{{ asset('assets/products/' . $product->firstMedia->file_name) }}" alt="{{$product->name}}"></a>
                                        <div class="product-overlay">
                                            <ul class="mb-0 list-inline">
                                                <li class="list-inline-item m-0 p-0"><a wire:click.prevent="addToWishList('{{$product->id}}')" class="btn btn-sm btn-outline-dark" href="javascript:void(0);"><i class="far fa-heart"></i></a></li>
                                                <li class="list-inline-item m-0 p-0"><a wire:click.prevent="addToCart('{{$product->id}}')" class="btn btn-sm btn-dark" href="javascript:void(0);">Add to cart</a></li>
                                                <li class="list-inline-item mr-0"><a wire:click.prevent="$emit('showProductModalAction', '{{$product->slug}}')" class="btn btn-sm btn-outline-dark" href="javascript:void(0);" data-target="#productView" data-toggle="modal"><i class="fas fa-expand"></i></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <h6> <a class="reset-anchor" href="{{'frontend.product' , $product->slug}}">{{ $product->name}}</a></h6>
